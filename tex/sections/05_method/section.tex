...
本文之所以采用 ResNet，并不只是因为它是经验上常见的视觉骨干，更因为其残差参数化（residual parameterization）在优化景观（optimization landscape）层面提供了更稳定的训练几何。对于深层网络，若直接学习某一层的完整变换 $H(x)$，优化过程往往需要在高度非凸（non-convex）且条件数较差（ill-conditioned）的参数空间中同时协调恒等传递与增量修正。而 ResNet 通过将目标改写为
\begin{equation}
    H(x) = x + F(x),
\end{equation}
把困难问题重写为围绕恒等映射（identity mapping）的扰动学习（perturbation learning）。

这种重参数化（reparameterization）并不会改变模型的表达能力上界，却会显著改变梯度传播路径与局部曲率结构，使深层网络更容易沿着“接近恒等、逐步偏移”的方向被优化。换言之，残差连接（skip connection）把原本容易退化（degradation）的深层拟合问题，转化为一类更平滑、可达性更强的优化问题，这也是本文将其视为优化层面基座（optimization-oriented backbone）的原因。

对于组织病理图像而言，这一点尤其重要。IDC patch 的判别信息通常表现为局部纹理、腺体结构、染色差异与细胞排列等多尺度形态学线索（multi-scale morphological cues）的组合，而非单一、线性的低层特征。因此，我们希望主干网络既能保持较深层次的表征能力，又不因深度增加而显著恶化训练动力学（training dynamics）。ResNet 正适合承担这一角色：它允许网络在浅层统计模式之上逐层叠加残差修正，从而在保留稳定优化路径的同时，逐步形成更贴近病灶判别边界的特征表征。
