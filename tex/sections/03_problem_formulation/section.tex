...
基于以上定义，本文研究的核心问题可以概括为：在患者级隔离约束下，给定训练子集 $\mathcal{D}_{\mathrm{train}}$，学习参数 $\theta$ 并选择决策阈值 $\tau$，使所得判别器在经验风险意义下拟合训练数据，同时在未见患者构成的验证子集 $\mathcal{D}_{\mathrm{val}}$ 上保持稳定性能。形式上，我们关心的不是
\begin{equation}
    (x,y,p) \sim \mathcal{D}_{\mathrm{train}},
\end{equation}
上的记忆能力本身，而是当
\begin{equation}
